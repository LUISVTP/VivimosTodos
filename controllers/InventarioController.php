...
if ($accion == 'crear' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO insumos (nombre, descripcion, cantidad, estado, marca) VALUES (?, ?, ?, ?, ?)");
    if ($stmt->execute([$cat['nombre'], $descripcion, $cantidad, $estado, $marca])) {
...
if ($accion == 'actualizar' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($rol_id != 1) { header("Location: ../views/admin/inventario.php?error=Acción no permitida."); exit; }
    //el metodo post recibe lo datos que se ingresan
