...
// Reservas aprobadas próximas (semáforo punto 10)
$hoy = new DateTime();
$stmt_proximas = $conn->query(
    "SELECT r.id, u.nombre_completo, r.fecha_evento, r.descripcion
     FROM reservas r
     INNER JOIN usuarios u ON r.usuario_id = u.id
     WHERE r.estado = 'aprobada'
       AND r.fecha_evento >= CURRENT_DATE
       AND r.fecha_evento <= CURRENT_DATE + INTERVAL '7 days'
     ORDER BY r.fecha_evento ASC"
);
$proximas = $stmt_proximas->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<div class="container-fluid px-4">
    ...
    <!-- ══ SEMÁFORO DE RESERVAS PRÓXIMAS (Punto 10) ══ -->
    <?php if (!empty($proximas)): ?>
    <div class="card shadow border-0 mb-4">
        <div class="card-header fw-bold" style="background:#1a1a2e;color:#fff;">
            <i class="bi bi-traffic-light"></i> Semáforo — Reservas Aprobadas Próximas a Vencer
        </div>
        <div class="card-body p-0">
            <table class="table table-hover align-middle mb-0">
                ...
                <?php foreach ($proximas as $p):
                    $evento   = new DateTime($p['fecha_evento']);
                    $dias     = (int)$hoy->diff($evento)->format('%r%a');
                    if ($dias <= 0) {
                        $clase = 'table-danger';
                        $icono = '🔴';
                        $texto = 'HOY';
                ...
                <tr class="<?php echo $clase; ?>">
                    ...
                    <td>
                        <span class="badge <?php echo ($dias <= 3) ? 'bg-danger' : (($dias <= 7) ? 'bg-warning text-dark' : 'bg-success'); ?> px-3 py-2">
                            <?php echo $texto; ?>
                        </span>
                    </td>
                    <td>
                        ...
                        <?php foreach ($usuarios as $fila): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="../../controllers/UsuarioController.php?accion=estado&id=<?php echo $fila['id']; ?>&estado_actual=<?php echo $fila['estado']; ?>"
                                        class="btn btn-sm btn-warning">
                                        <i class="bi bi-toggle-on"></i> Estado
                                    </a>
                                    <a href="../../controllers/UsuarioController.php?accion=eliminar&id=<?php echo $fila['id']; ?>"
