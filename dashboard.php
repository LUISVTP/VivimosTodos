...
// Validar que el usuario esté logueado y sea Administrador (rol_id = 1)
if (!isset($_SESSION['usuario_id']) || $_SESSION['rol_id'] != 1) {
    header("Location: index.php?error=Acceso denegado. Solo administradores.");
    exit();
}

// Obtener la lista de usuarios con sus roles (Añadido u.rol_id para el modal de edición)
        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav me-auto">
                <?php if($_SESSION['rol_id'] == 1): ?>
                    <li class="nav-item">
                        <a class="nav-link active" href="dashboard.php">Usuarios</a>
                    </li>
                <?php endif; ?>
                <li class="nav-item">
                    <a class="nav-link" href="inventario.php">Inventario Salón Social</a>
                </li>
            </ul>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-info text-white" data-bs-toggle="modal" data-bs-target="#modalEditar<?php echo $fila['id']; ?>">
                                        Editar
                                    </button>

                                    <a href="acciones_usuario.php?accion=estado&id=<?php echo $fila['id']; ?>&estado_actual=<?php echo $fila['estado']; ?>" class="btn btn-sm btn-warning">Estado</a>
                                    
                                    <a href="acciones_usuario.php?accion=eliminar&id=<?php echo $fila['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Estás seguro de eliminar este usuario?');">Eliminar</a>
                                </td>
                            </tr>

                            <div class="modal fade" id="modalEditar<?php echo $fila['id']; ?>" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                  <form action="acciones_usuario.php" method="POST">
                                    ...
                                    <div class="modal-header bg-info text-white">
                                      <h5 class="modal-title">Editar Usuario: <?php echo htmlspecialchars($fila['nombre_completo']); ?></h5>
                                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label class="form-label">Nombre Completo</label>
                                            <input type="text" name="nombre" class="form-control" value="<?php echo htmlspecialchars($fila['nombre_completo']); ?>" required>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Correo Electrónico</label>
