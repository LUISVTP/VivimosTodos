...
// ── Filtros de gestión ───────────────────────────────────────────
$g_estado = $_GET['g_estado'] ?? 'todos';
$g_desde  = $_GET['g_desde']  ?? '';
$g_hasta  = $_GET['g_hasta']  ?? '';

// ── Reservas según rol + filtros via API REST ─────────────────────
$usuarios_all = sb_get('usuarios', [], ['id', 'nombre_completo']);
$usr_map = [];
foreach ($usuarios_all as $u) $usr_map[$u['id']] = $u['nombre_completo'];

if ($rol_id == 1 || $rol_id == 3) {
    $filtros_g = [];
    if ($g_estado !== 'todos') $filtros_g['estado'] = 'eq.' . $g_estado;
    if ($g_desde)              $filtros_g['fecha_evento'] = 'gte.' . $g_desde;
    if ($g_hasta)              $filtros_g['fecha_evento'] = 'lte.' . $g_hasta;
    $reservas_raw = sb_get('reservas', $filtros_g);
    // Filtro rango combinado en PHP
    if ($g_desde && $g_hasta) {
        $reservas_raw = array_filter($reservas_raw, fn($r) => $r['fecha_evento'] >= $g_desde && $r['fecha_evento'] <= $g_hasta);
    }
} else {
    $reservas_raw = sb_get('reservas', ['usuario_id' => 'eq.' . $usuario_id]);
}
// Join manual con usuarios
...
$reservas = array_values($reservas_raw);

// ── Insumos disponibles (solo residente) ─────────────────────────
$insumos_disponibles = [];
if ($rol_id == 2) {
    $ins_d = sb_get('insumos', ['estado' => 'eq.disponible'], ['id','nombre','cantidad']);
    $insumos_disponibles = array_values(array_filter($ins_d, fn($i) => $i['cantidad'] > 0));
...
if ($rol_id == 1 || $rol_id == 3) {
    // ── Estadísticas via API REST ─────────────────────────────────
    $all_r2  = sb_get('reservas', [], ['id','estado','fecha_evento','usuario_id','descripcion']);
    $all_i2  = sb_get('insumos',  [], ['id','estado']);
    $total_reservas  = count($all_r2);
    $pendientes_cnt  = count(array_filter($all_r2, fn($r) => $r['estado'] === 'pendiente'));
    $aprobadas_cnt   = count(array_filter($all_r2, fn($r) => $r['estado'] === 'aprobada'));
    $rechazadas_cnt  = count(array_filter($all_r2, fn($r) => $r['estado'] === 'rechazada'));
    $total_insumos   = count($all_i2);
    $insumos_danados = count(array_filter($all_i2, fn($i) => $i['estado'] === 'dañado'));

    // Gráfica 1: reservas por mes
    $seis_m = date('Y-m-d', strtotime('-6 months'));
    $m_count = [];
    foreach ($all_r2 as $r) {
        if ($r['fecha_evento'] >= $seis_m) {
            $k = date('M Y', strtotime($r['fecha_evento']));
            $m_count[$k] = ($m_count[$k] ?? 0) + 1;
        }
    }
    $labels_mes2 = json_encode(array_keys($m_count));
    $values_mes2 = json_encode(array_values($m_count));

    // Gráfica 2: estados
    $labels_est2 = json_encode(['Pendientes','Aprobadas','Rechazadas']);
    $values_est2 = json_encode([$pendientes_cnt, $aprobadas_cnt, $rechazadas_cnt]);

    // Gráfica 3: inventario con colores correctos
    $i_count = [];
    foreach ($all_i2 as $i) $i_count[$i['estado']] = ($i_count[$i['estado']] ?? 0) + 1;
    ksort($i_count);
    $labels_inv2     = json_encode(array_keys($i_count));
    $values_inv2     = json_encode(array_values($i_count));
    $cmap2           = ['disponible' => '#198754','dañado' => '#dc3545','en reparacion' => '#ffc107'];
    $colors_inv2     = json_encode(array_map(fn($k) => $cmap2[$k] ?? '#6c757d', array_keys($i_count)));

    // Vista previa con filtros
    $filtro_e = $_GET['filtro_estado'] ?? 'todos';
    $filtro_d = $_GET['filtro_fecha_desde'] ?? '';
    $filtro_h = $_GET['filtro_fecha_hasta'] ?? '';
    $u2_arr   = sb_get('usuarios', [], ['id','nombre_completo']);
    $u2_map   = [];
    foreach ($u2_arr as $u) $u2_map[$u['id']] = $u['nombre_completo'];
    $preview  = array_filter($all_r2, function($r) use ($filtro_e, $filtro_d, $filtro_h) {
        if ($filtro_e !== 'todos' && $r['estado'] !== $filtro_e) return false;
        if ($filtro_d && $r['fecha_evento'] < $filtro_d) return false;
        if ($filtro_h && $r['fecha_evento'] > $filtro_h) return false;
        return true;
    });
    usort($preview, fn($a, $b) => strcmp($b['fecha_evento'], $a['fecha_evento']));
    $preview = array_slice(array_values($preview), 0, 10);
    foreach ($preview as &$p) $p['nombre_completo'] = $u2_map[$p['usuario_id']] ?? 'Desconocido';
    unset($p);

    // Alias para tarjetas en HTML
    $pendientes  = $pendientes_cnt;
    $aprobadas   = $aprobadas_cnt;
    $rechazadas  = $rechazadas_cnt;
    // Alias gráficas
    $labels_mes = $labels_mes2; $values_mes = $values_mes2;
    $labels_est = $labels_est2; $values_est = $values_est2;
    $labels_inv = $labels_inv2; $values_inv = $values_inv2;
    $colors_inv_json = $colors_inv2;
}
...
<div class="container-fluid px-4">
    ...
    <div class="row">
        ...
        <div class="<?php echo ($rol_id == 2) ? 'col-md-8' : 'col-12'; ?>">
            ...
            <!-- Filtros de gestión (solo admin y supervisor) -->
            <?php if ($rol_id == 1 || $rol_id == 3): ?>
            <div class="card shadow mb-3">
                <div class="card-body py-2">
                    <form method="GET" action="reservas.php" class="row g-2 align-items-end">
                        <input type="hidden" name="tab" value="gestion">
                        <div class="col-auto">
                            <label class="form-label fw-semibold small mb-1">Estado</label>
                            <select name="g_estado" class="form-select form-select-sm" style="min-width:140px">
                                <option value="todos"     <?php echo ($g_estado==='todos')     ? 'selected':''; ?>>Todos</option>
                                <option value="pendiente" <?php echo ($g_estado==='pendiente') ? 'selected':''; ?>>Pendientes</option>
                                <option value="aprobada"  <?php echo ($g_estado==='aprobada')  ? 'selected':''; ?>>Aprobadas</option>
                                <option value="rechazada" <?php echo ($g_estado==='rechazada') ? 'selected':''; ?>>Rechazadas</option>
                            </select>
                        </div>
                        <div class="col-auto">
                            <label class="form-label fw-semibold small mb-1">Desde</label>
                            <input type="date" name="g_desde" class="form-control form-control-sm"
                                   value="<?php echo htmlspecialchars($g_desde); ?>">
                        </div>
                        <div class="col-auto">
                            <label class="form-label fw-semibold small mb-1">Hasta</label>
                            <input type="date" name="g_hasta" class="form-control form-control-sm"
                                   value="<?php echo htmlspecialchars($g_hasta); ?>">
                        </div>
                        <div class="col-auto d-flex gap-2 align-items-end">
                            <button type="submit" class="btn btn-sm btn-dark fw-bold px-3">
                                <i class="bi bi-search"></i> Filtrar
                            </button>
                            <a href="reservas.php?tab=gestion" class="btn btn-sm btn-outline-secondary px-3">
                                <i class="bi bi-x"></i> Limpiar
                            </a>
                        </div>
                        <?php if ($g_estado !== 'todos' || $g_desde || $g_hasta): ?>
                        <div class="col-auto d-flex align-items-end">
                            <span class="badge bg-primary px-3 py-2">
                                <?php echo count($reservas); ?> resultado(s)
                            </span>
                        </div>
                        <?php endif; ?>
                    </form>
...
new Chart(document.getElementById('chartEstados'), {
    type: 'doughnut',
    data: {
        labels: <?php echo $labels_est; ?>,
        datasets: [{
            data: <?php echo $values_est; ?>,
...
new Chart(document.getElementById('chartInventario'), {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: <?php echo $colors_inv_json; ?>,
            borderRadius: 6,
        }]
    },
    options: {
