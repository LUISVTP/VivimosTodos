...
// ── Contadores y datos via API REST ────────────────────────────────
$all_res    = sb_get('reservas', [], ['id','estado','fecha_evento','usuario_id','descripcion','fecha_solicitud']);
$all_ins    = sb_get('insumos',  [], ['id','estado']);

$total_reservas  = count($all_res);
$pendientes      = count(array_filter($all_res, fn($r) => $r['estado'] === 'pendiente'));
$aprobadas       = count(array_filter($all_res, fn($r) => $r['estado'] === 'aprobada'));
$rechazadas      = count(array_filter($all_res, fn($r) => $r['estado'] === 'rechazada'));
$total_insumos   = count($all_ins);
$insumos_danados = count(array_filter($all_ins, fn($i) => $i['estado'] === 'dañado'));

// ── Gráfica 1: reservas por mes (últimos 6 meses) ──────────────────
$seis_meses = date('Y-m-d', strtotime('-6 months'));
$meses_count = [];
foreach ($all_res as $r) {
    if ($r['fecha_evento'] >= $seis_meses) {
        $key = date('M Y', strtotime($r['fecha_evento']));
        $meses_count[$key] = ($meses_count[$key] ?? 0) + 1;
    }
}
$labels_mes = json_encode(array_keys($meses_count));
$values_mes = json_encode(array_values($meses_count));

// ── Gráfica 2: torta de estados ────────────────────────────────────
$labels_estados = json_encode(['Pendientes', 'Aprobadas', 'Rechazadas']);
$values_estados = json_encode([$pendientes, $aprobadas, $rechazadas]);

// ── Gráfica 3: inventario por estado con colores correctos ─────────
$inv_count = [];
foreach ($all_ins as $i) $inv_count[$i['estado']] = ($inv_count[$i['estado']] ?? 0) + 1;
ksort($inv_count);
$labels_inv = json_encode(array_keys($inv_count));
$values_inv = json_encode(array_values($inv_count));

// ── Filtros vista previa ────────────────────────────────────────────
...
// Traer usuarios para join manual
$usr_arr = sb_get('usuarios', [], ['id','nombre_completo']);
$usr_map = [];
foreach ($usr_arr as $u) $usr_map[$u['id']] = $u['nombre_completo'];

// Aplicar filtros en PHP
$preview = array_filter($all_res, function($r) use ($filtro_estado, $filtro_fecha_desde, $filtro_fecha_hasta) {
    if ($filtro_estado !== 'todos' && $r['estado'] !== $filtro_estado) return false;
    if ($filtro_fecha_desde && $r['fecha_evento'] < $filtro_fecha_desde) return false;
    if ($filtro_fecha_hasta && $r['fecha_evento'] > $filtro_fecha_hasta) return false;
    return true;
});
usort($preview, fn($a, $b) => strcmp($b['fecha_evento'], $a['fecha_evento']));
$preview = array_slice(array_values($preview), 0, 10);
foreach ($preview as &$p) $p['nombre_completo'] = $usr_map[$p['usuario_id']] ?? 'Desconocido';
unset($p);
?>
<!DOCTYPE html>
<html lang="es">
